...
parser = MyParser(
    ...
    description=("This script helps you clean up your simulations. It's pretty "
                 "elaborate, because Gromacs has a lot of ways it can screw with "
                 "simulation files. Normally files are always checked before "
                 "some action is taken. If the program gets interrupted at some "
                 "point it will continue where it left. So iterative calls are "
                 "encouraged."),
...
parser.add_argument(
    ...
    dest="directories",
    help=("The input directories. You can provide as many as you like. You can "
          "include truncation marks similar to rsync in the directory names. "
          "These truncation marks will be used to determine the folder structure "
          "in the output_directory. Let's say you provide "
          "./cleanup_sims.py -d /path/to/./first/simulation/xtcs -d"
          "/path/to/some/other/./simulation -o /output/dir, the directories "
          "/output/dir/first/simulation/xtcs and /output/dir/simulation "
          "will be created."),
)
parser.add_argument(
    "-o",
    ...
    dest="out_dir",
    help=("The output directory, if you use the `-trjcat` flag, only the final "
          "concatenated file will be written to the folders in the output "
          "directory. The cleaned partial xtc files will be put into the same "
          "directory they are now in. If `-trjcat` is not given, all *xtc files "
...
parser.add_argument(
    ...
    help=("Similar to Gromacs' dt option. Only writes frames every frame mod "
          ...
          "`gmx trjconv`. However, sometimes gromacs screws up and makes a dt "
          "100 ps to some dt 92 and some dt 8 ps. If -1 is given all frames will "
          "be written to output. Defaults to -1.")
)
parser.add_argument(
    "-max",
    default=-1,
    ...
    help=("The maximum time in ps to write trajectories. If some of your "
          "simulations don't reach that time, an exception will be thrown. If "
          "-1 is provided, the maximum time per xtcs in a directoyr is used. "
...
parser.add_argument(
    ...
    help=("Number of atoms that should be in the cleaned xtc files. If files "
          "are already present in the output directory and don't match the "
          "requested n_atoms, they will be overwritten."),
)
parser.add_argument(
    "-s",
    default="topol.tpr",
...
parser.add_argument(
    ...
    help=("The .xtc files in the directories. Similar to gromacs' -f flag (which "
          "is -x in mdrun). Will overwrite the values set with -deffnm. So "
          "setting -deffnm production -x my_traj.xtc will look for my_traj.xtc, "
          "my_traj.part0001.xtc, my_traj.part0002.xtc and so on in the "
          "simulation directories. Defaults to traj_comp.xtc."),
)
parser.add_argument(
    "-pbc",
    default="none",
    metavar="nojump, res, mol, whole, cluster, none",
    help=("What to provide for the periodic boundary correction of `trjconv`."
          ...
          "The pbc method will be used as the name of the output file. So `-pbc` "
          "nojump will produce traj_nojump.xtc in your outout directories "
...
parser.add_argument(
    "-center",
    action="store_true",
    help=("Similar to gromacs trjconv's -[no]center option. If center is "
          "provided the -ndx-group will be used both for centering and output. "
          "Use the python function and provide a string with newline character "
...
parser.add_argument(
    ...
    help=("The string to provide for `gmx trjconv` to center and remove pbcs from. "
          "Can either be an integer (0 is most of the times the system, 1 is "
          "most of the times the protein) or a string like System, Protein, "
          "or a custom group read from the ndx file, which is created if "
          "`-create-ndx` is provided. In any case, if you provide `-n-atoms`, "
          "the algorithm will check the output and inform you, when it contains "
          "a different number of atoms. This will allow you to tweak your "
          "group selection or make sure, that Gromacs recognizes your protein "
          "in group 1 correctly."),
)
parser.add_argument(
    "-deffnm",
    default=None,
    ...
    help=("The default filename for the files in the `-d` input directories. "
          "If you run your mdrun simulations with `-deffnm` production, you "
          "should also provide production for this argument. "
          "If `-s` or `-x` are set, this will be overwritten."),
)
parser.add_argument(
    "-trjcat",
    action="store_true",
    help=("Whether to concatenate the trajectories from the input directories "
          "into one long (`-max` ps) trajectory. If `-trjcat` is set, the output "
          "directory will only contain one .xtc file. The outputs from `gmx "
          "trjconv` will be written into the input directories along with the "
          "input .xtc files."),
)
parser.add_argument(
    "-create-pdb",
    action="store_true",
    help=("When given, the output directories will also contain start.pdb files "
          "that are extracted from the first frame of the simulations. These "
          "can be used to load the clean trajectries into other tools."),
)
parser.add_argument(
    "-create-ndx",
    action="store_true",
    help=("If gromacs doesn't recognize your protein as such and the index "
          "group 1 (Protein) contains the wrong number of atoms, you can create "
          "index.ndx files in the input directories with this option. "
          "See the `-ndx-group-in` flag how to do so."),
)
parser.add_argument(
    "-ndx-group-in",
    default=None,
    ...
    help=("If you have non-standard residues in your protein and they are not "
          "included in group 1 (protein) of the standard index, you can add a "
          "custom group using this flag. If you have two non-standard residues "
          "(LYQ and GLQ) you can create a new group from the protein and the "
          "residue indices by providing the string 'Protein | GLQ | LYQ' (these "
          "are logical or). This will use gmx make_ndx and the simulations .tpr "
          "file to create an index.ndx file. The `-ndx-group` flag should then "
          "be 'Protein_GLQ_LYQ'. If you are not sure, what to provide here, "
          "play around with your .tpr files and `gmx make_ndx` and then start this "
          "program with what you've learned from there."),
)
parser.add_argument(
    "-per-file-timestep-policy",
    default="raise",
    help=("Currently not used. The idea is to raise an Exception, if a -dt "
          "is not possible. Example: The file traj_comp.xtc has coordinates "
          "every 20 ps but -dt 15 was provided. This should include some logic "
          "to offer alternatives."),
)
parser.add_argument(
    "-inter-file-timestep-policy",
    default="raise",
    help=("Currently not in use. Should contain logic on how to deal "
          "discontinuities between trajectory files."),
)
parser.add_argument(
    "-file-exists-policy",
    default="raise",
    metavar="raise, overwrite, continue, check_and_continue, check_and_overwrite",
    help=("What to do if a file already exists. Let's say the algorithm tries "
          "to overwrite traj_nojump.xtc, but it already exists. If 'raise' is "
          "provided, the algorithm will terminate and raise an exception, if "
          "'overwrite' is provided, the file will be overwritten without "
          "additional checks. If 'continue' if provided, the file is assumed to "
          "be good (this can lead to unforeseen consequences ie. different "
          "number of atoms in files, etc). If 'check_and_continue' is provided, "
          "the file will be checked. If it is not ok, an exception will be "
          "raised. If 'check_and_overwrite' is provided the file will only be "
          "overwritten, if it is wrong (i.e. wrong `-dt`, wrong `-n-atoms`)."),
)
parser.add_argument(
    "-clean-copies",
    action="store_true",
    help=("Currently not in use anymore. Intention was to clean gromacs "
          "copy files (#traj_comp.part0002.xtc.4#), but -file-exists-policy "
          "replaced that part."),
)
parser.add_argument(
    "-dry-run",
...
parser.add_argument(
    ...
    help=("Where to log to. The logfile contains a lot of info. Especially, "
          "if something happens which is not logged to the default WARNING level."),
)
parser.add_argument(
    "-loglevel",
    default="WARNING",
...
def map_in_and_out_files(
    ...
    # if pbc is none, we will use clean for that
    if pbc == "none":
        pbc = "clean"
    ...
    for directory in directories:
        ...
        if combine:
            key_dir = Path(directories[0])
        else:
            key_dir = Path(directory)

        # create the key if it not exists, if we combine, we don't overwrite here
        if not combine or key_dir not in mapped_sims:
            mapped_sims[key_dir] = {}
        ...
        if len(files) == 1:
            ...
            if "/./" in str(directory):
                ...
                out_file = out_dir_ / file.name.replace(
        ...
        # put the tmp sims into input, then cat
            ...
            for file in files:
                ...
                mapped_sims[key_dir][file] = out_file
    return mapped_sims
...
@Semaphore(10)
async def _get_times_from_file(
    file: Path,
...
async def get_times_from_file(
    ...
    assert file.exists()
    try:
        times = await asyncio.wait_for(_get_times_from_file(file, logger=logger), timeout=timeout)
    except TimeoutError as e:
        raise Exception(
            f"Could not read times from {file=}, within the timeout={timeout} s."
            f"{file.resolve()=}, {file.exists()=}"
        ) from e
    return times
    ...
    for file in input_files:
        ...
        with XTCReader(str(file)) as r:
            for i, ts in enumerate(r.trajectory):
                print(ts.time)
                if i == 0:
                    prev_time = deepcopy(ts.time)
                    prev_pos = deepcopy(ts.positions)
                    continue
                dt = int(ts.time - prev_time)
                unique_dt.append(dt)
                if dt not in timestep_drift[file]:
                    timestep_drift[file][dt] = []
                drift = np.mean(np.abs(np.linalg.norm(prev_pos - ts.positions)))
                mean_all.append(drift)
                timestep_drift[file][dt].append(drift)
                prev_time = deepcopy(ts.time)
                prev_pos = deepcopy(ts.positions)
                if i >= 500 and prelim_screen:
                    break

        # mean drift per dt per file
        for dt in timestep_drift[file].keys():
            timestep_drift[file][dt] = np.mean(timestep_drift[file][dt])

    # mean all
    mean_all = np.mean(mean_all)
    drift_ok = np.all([np.all([np.isclose(v, mean_all, rtol=acceptable_drift) for v in value.values()]) for value in timestep_drift.values()])
    timesteps_ok = len(timesteps_file) == 1

    # Not all drift_ok and timestep_ok combinations are possible
    # case 1 (all good): The timesteps are regular and the atomic drift is regular, too:
    # timetsep_ok and drift_ok
    # => continue
    # case 2 (gromacs wrote wrong timesteps): Different timesteps occur, but the
    # atomic drift between those timesteps does not differ
    # not timesteps_ok and drift_ok
    # case 3 (timesteps differ, but drifts fit for their specific timestep bracket)
    #
    ...
    if dt != -1:
        ...
        if n_timesteps != n_timesteps_in_files:
            logger.warning(
                ...
                f"This might not be the end of this simulation I will continue. "
                f"to check whether the coordinates or the times are wrong"
            )
            return False
        ...
        if i != 0:
            ...
            if len(times) == 1:
                logger.debug(
                    ...
                    f"safe to discard such files in the algorithm. Because this "
                    f"timestep will be in another file."
                )
                out.pop(file)
                continue
            ...
            if end_dt < previous_end_dt and start_dt < previous_end_dt:
                ...
                if (diff := start_dt - previous_end_dt) > dt:
                    msg = (
                        ...
                        f"requested {dt=}. The difference is {diff=}. "
                        f"The last times of the previous file: "
                        f"\n{previous_times[-50:]=}\nand the first times in file: "
                        f"\n{times[:50]=}\n. This is a reason to stop the algorithm here."
                    )
                    raise Exception(msg)
                elif (diff := start_dt - previous_end_dt) == 0:
                    start_dt = previous_end_dt + 100
                    logger.info(
                        f"The difference between {previous_file=} and {file=} is "
                        f"negative {start=}-{previous_end=}="
                        f"{(start - previous_end)} ps. And the diff in dt is zero:"
                        f"{diff=}. This means, I need to skip some "
                        f"frames at the start of {file=}. The previous file ends "
                        f"with a timestamp of {previous_times[-1]=}, which in dt is "
                        f"{previous_end_dt=}. The new file starts at a time "
                        f"{times[0]}, but due to the difference being negative, I "
                        f"will let it start at {start_dt=}."
                    )
                else:
                    msg = (
...
async def write_and_check_times(
    ...
    if not data_file.is_file():
        ...
        times = await asyncio.gather(*[get_times_from_file(s, logger=logger) for s in input_files])
        times = {k: v for k, v in zip(input_files, times)}

        # frames and times of maybe existing output files
        logger.debug(f"Adding destination files dor {sim_dir} if available.")
        _ = await asyncio.gather(*[get_times_from_file(s, logger=logger) for s in output_files])
        ...
        if all_out_file:
            ...
            _ = await get_times_from_file(all_out_file, logger=logger)
...
async def create_ndx_file(
    ...
    if not _dryrun:
        ...
        if not ndx_file.is_file():
            raise Exception(
                f"Could not create file {ndx_file}. {proc.returncode=}, "
                f"{stderr.decode()=}, {stdout.decode()=}, {cmd=}"
            )
        else:
            logger.info(f"Created {ndx_file}")
    else:
...
async def run_command_and_check(
    ...
    if not _dryrun:
        ...
        if not out_file.is_file():
            raise Exception(
                f"Could not create file {out_file}. {proc.returncode=}, "
                f"{stderr.decode()=}, {stdout.decode()=}, {cmd=}"
            )
        else:
    ...
    if not _dryrun:
        ...
        if not tmp_tpr.is_file():
            raise Exception(
                ...
                f"{proc.returncode=}, {stderr.decode()=}, {stdout.decode()=}, {cmd=}."
            )
        else:
...
async def run_concat_command(
    ...
    if not _dryrun:
        ...
        timesteps = np.array([dt])
        start_ok = True
        end_ok = True
        timestep_ok = np.array([dt])
    if len(timestep_ok) == 1:
        timestep_ok = timestep_ok[0]
        timestep_ok = timestep_ok == dt
    else:
...
async def async_create_pdb(
    ...
    if not _dryrun:
        ...
        if not pdb_out_file.is_file():
            raise Exception(
                f"Could not create file {pdb_out_file}. {proc.returncode=}, "
                f"{stderr.decode()=}, {stdout.decode()=}, {pdb_cmd['cmd']=}"
            )
        else:
            logger.info(f"Created {pdb_out_file}")
    else:
...
async def async_run_create_tpr(
    tpr_commands: list[dict[str, Union[str, Path, int]]],
    n_atoms: int = -1,
    file_exists_policy: FileExistsPolicyType = "raise",
    logger: Optional[logging.Logger] = None,
) -> None:
    await asyncio.gather(
        *[
            async_create_tpr(tpr_cmd, n_atoms, file_exists_policy, logger=logger)
            for tpr_cmd in tpr_commands
        ]
    )


async def async_create_tpr(
    tpr_cmd: dict[str, Union[str, Path, int]],
    n_atoms: int = -1,
    file_exists_policy: FileExistsPolicyType = "raise",
    logger: Optional[logging.Logger] = None,
) -> None:
    tpr_out_file = tpr_cmd["out_file"]
    check = False
    if tpr_out_file.is_file():
        if file_exists_policy == "raise":
            raise Exception(
                f"File {tpr_out_file} already exists. "
                f"Due to the chosen `{file_exists_policy=}` "
                "I have raised an Exception."
            )
        elif file_exists_policy == "overwrite":
            logger.debug(f"Will overwrite {tpr_out_file} without checking.")
            overwrite = True
        elif file_exists_policy == "continue":
            logger.debug(f"File {tpr_out_file} already exists. Continuing")
            return
        elif file_exists_policy == "check_and_overwrite":
            logger.debug(f"Will overwrite {tpr_out_file} when checks fail.")
            overwrite = True
            check = True
        elif file_exists_policy == "check_and_continue":
            logger.debug(f"Will check {tpr_out_file} and raise, when checks fail.")
            overwrite = False
            check = True
        else:
            raise Exception(f"Unkown file_exists_policy: {file_exists_policy=}")
    ...
    if check and n_atoms != -1:
        if not _dryrun:
            n_atoms_in_file = mda.Universe(str(tpr_out_file)).atoms.n_atoms
        else:
            n_atoms_in_file = n_atoms
        if n_atoms_in_file != n_atoms:
            if not overwrite:
                raise Exception(
                    f"{tpr_out_file} has the wrong number of atoms. "
                    f"{n_atoms} was requested, but the file contains "
                    f"{n_atoms_in_file} atoms. "
                    f"Set `file_exists_policy` to 'overwrite' or "
                    f"'check_and_overwrite' to overwrite this file."
                ...
                logger.debug(
                    f"{tpr_out_file} indexes the wrong number of atoms. "
                    f"{n_atoms} was requested, but the file contains "
                    f"{n_atoms_in_file} atoms. "
                    f"I will overwrite this file."
                )
            ...
            logger.debug(
                f"{tpr_out_file} is fine. It has the correct "
                f"number of atoms: {n_atoms}."
            )
            return
    ...
    if overwrite:
        if not _dryrun:
            tpr_out_file.unlink(missing_ok=True)
        else:
            logger.warning(f"DRY-RUN: Deleted {tpr_out_file}")
    ...
    if not _dryrun:
        ...
        stdout, stderr = await proc.communicate(tpr_cmd["stdin"].encode())
        if not tpr_out_file.is_file():
            raise Exception(
                f"Could not create file {tpr_out_file}. {proc.returncode=}, "
                f"{stderr.decode()=}, {stdout.decode()=}, {pdb_cmd['cmd']=}"
            )
        else:
            logger.info(f"Created {tpr_out_file}")
    else:
        logger.warning(f"DRY-RUN: Created {tpr_out_file}")
    ...
    if n_atoms != -1:
        if n_atoms_in_file != n_atoms:
            raise Exception(
                f"{tpr_out_file} has the wrong number of atoms. "
                f"{tpr_out_file} was requested, but the file contains "
                f"{n_atoms_in_file}. "
                f"You can try and provide a different `output_group_and_center` "
                f"(-ndx-group for the CLI)."
                f"Try to find the correct stdin by calling the command: "
                f"{tpr_cmd['cmd']} manually."
            )
    ...
    * Keeping track of files on disk via imohash. If a file changes through some
    ...
    Args:
        ...
        pbc (str): What to provide for the periodic boundary correction of
            ...
            that in theory my_traj_file_nojump.xtc and
            my_traj_file_nojump.part0001.xtc are possible.
        ur (str): The unit cell representation. Refer to the gromacs manual for
            this setting. Defaults to "rect".
        ...
        create_pdb (bool): When given, the output directories will also contain
            ...
            other tools. Defaults to True.
        create_tpr (bool): When given, the output directories will also contain
            topol.tpr files that are extracted from the simulation's tpr files.
            These contain bonds and interaction parameters and can be used to
            load the bonds of the trajectory, if your program of choice does
            not recognize the bonds in the start.pdb. Defaults to True.
        create_ndx (bool): If gromac's doesn't recognize your protein as such
            ...
            option. See the -ndx-group-in flag how to do so. Defaults to False.
        ...
        loglevel (int): What level to log at. Defaults to WARNING.
    ...
    if create_pdb or create_tpr:
        ...
        for sim_dir, simulation in simulations.items():
            ...
            tpr_out_file = out_file.parent / "topol.tpr"
            pdb_cmd = (
                f"gmx trjconv -f {out_file} -s {tpr_file} -o {pdb_out_file} -dump 0 -ur {ur}"
            )
            tpr_cmd = (
                f"gmx convert-tpr -s {tpr_file} -o {tpr_out_file}"
            )
            if center:
                pdb_cmd += " -center"
            if create_ndx:
                pdb_cmd += f" -n {tpr_file.parent / 'index.ndx'}"
                tpr_cmd += f" -n {tpr_file.parent / 'index.ndx'}"
            pdb_cmd = {
                "cmd": pdb_cmd,
            ...
            tpr_cmd = {
                "cmd": tpr_cmd,
                "stdin": output_group_and_center,
                "out_file": tpr_out_file,
                "n_atoms": n_atoms,
            }
            if create_pdb:
                pdb_commands.append(pdb_cmd)
            if create_tpr:
                tpr_commands.append(tpr_cmd)
        ...
        for inp_file, command in plan.items():
            if inp_file == "trjcat":
                ...
                command = {
                    ...
                    f"-{center} -b {command['b']} -e {command['e']} -dt {command['dt']} -ur {ur}",
                    "b": command["b"],
                    "e": command["e"],
                    "dt": command["dt"],
                    "out_file": command["out_file"],
    ...
    if create_tpr:
        loop.run_until_complete(
            async_run_create_tpr(
                tpr_commands, n_atoms, file_exists_policy, logger=logger,
            )
        )
    ...
    if clean_copies:
        for sim_plan in plans:
            if "trjcat" in sim_plan:
                ...
                for inp_file, data in sim_plan.items():
                    output_dir = data["out_file"].parent
            ...
            for file in output_dir.glob("#*#"):
                if not _dryrun:
                    ...
                    # file.unlink()
                logger.critical(
                    f"The clean_copies feature has not been tested yet. "
